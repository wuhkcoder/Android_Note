...
public class DGSingleSelectDialog extends Dialog {
	...
	private void loadView() {
		mContentView = LayoutInflater.from(mActivity).inflate(
				R.layout.dg_dialog_singleselect, null);
	...
	public static class Builder {
		...
		public DGSingleSelectDialog create() {
			final DGSingleSelectDialog dialog = new DGSingleSelectDialog(
					activity);
			if (!Validators.isEmpty(itemTexts)) {
				dialog.mListView.setAdapter(new DGBaseAdapter() {
					@Override
					public View getView(int postion, View view, ViewGroup arg2) {
						if (null == view) {
							view = LayoutInflater.from(activity).inflate(
									R.layout.dg_dialog_singleselect_listitem,
									null);
						}
